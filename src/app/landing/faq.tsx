      ...
      <div className="mt-10 flex flex-col gap-6">
        {faq.map((f, index) => (
          <div
            ...
            className="shadow-custom rounded-3xl flex flex-col gap-4 p-4 cursor-pointer"
            key={index}
          >
            <div className="flex gap-4 items-start ">
              <p>{f.img}</p>
              <div className="flex flex-col gap-2">
                <h3 className="font-manrope-bold text-[16px] md:text-[18px] text-[#404040]">
                  {f.question}
                </h3>
                {open === index && (
                  <h4 className="font-manrope-regular text-[14px] md:text-[16px] text-[#404040]">
                    {f.answer}
                  </h4>
                )}
              </div>
